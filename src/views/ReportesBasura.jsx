// src/views/ReportesBasura.jsx - FRONTEND COMPONENT
import React, { useState, useEffect } from 'react';
import { ReportesBasuraController } from '../controllers/ReportesBasuraController';
...
const ReportesBasura = () => {
  ...
  const cargarDatosIniciales = async () => {
    ...
    // Cargar reportes
    const resultadoReportes = await ReportesBasuraController.cargarReportes();
    ...
    // Cargar brigadas
    const resultadoBrigadas = await ReportesBasuraController.cargarBrigadasDisponibles();
  ...
  const asignarBrigadaReporte = async (reporteId, brigadaId) => {
    ...
    const resultado = await ReportesBasuraController.asignarBrigadaReporte(reporteId, brigadaId);
  ...
  const desasignarReporte = async (reporte) => {
    ...
    const resultado = await ReportesBasuraController.desasignarReporte(
  ...
  const actualizarEstadoReporte = async (nuevoEstado) => {
    ...
    const resultado = await ReportesBasuraController.actualizarEstadoReporte(
  ...
  const generarPDF = async () => {
    ...
    const resultado = await ReportesBasuraController.generarPDF(reporteSeleccionado);
  ...
  const reportesFiltrados = ReportesBasuraController.filtrarReportes(
    reportes, 
    filtroEstado, 
    filtroFechaInicio, 
    filtroFechaFin
  );
  const estadisticas = ReportesBasuraController.calcularEstadisticas(reportesFiltrados);
  ...
  if (loading) {
    return (
      <div className="loading-container">
        ...
        <p>Cargando reportes de basura acumulada...</p>
      </div>
    );
  }
  ...
  return (
    <div className="reportes-app">
      ...
      <header className="app-header">
        <h1>Reportes de Basura Acumulada</h1>
      ...
      <div className="reportes-content">
        {reportesFiltrados.length === 0 ? (
          <div className="no-reports">
            <div className="no-reports-icon"><i className="fas fa-trash-alt"></i></div>
            <h3>No hay reportes de basura acumulada</h3>
              ...
              >
                ...
                <div className="report-card-body">
                  ...
                  <div className="report-fecha">
                    ...
                    <span>{ReportesBasuraController.formatearFecha(reporte.fecha)}</span>
                  </div>
        ...
        <div className="reporte-modal">
          <div className="reporte-modal-content">
            <div className="reporte-modal-header">
              <h2>Basura Acumulada - Folio: {reporteSeleccionado.folio}</h2>
            ...
            <div className="reporte-modal-body">
              <div className="reporte-detalles">
                <div className="reporte-campo">
                  ...
                  <div className="reporte-campo-valor">{ReportesBasuraController.formatearFecha(reporteSeleccionado.fecha)}</div>
                </div>
                      ...
                      {reporteSeleccionado.brigadaAsignada && (
                        <div className="brigada-asignada-info">
                          ...
                          <small className="fecha-asignacion">
                            Asignado el: {ReportesBasuraController.formatearFecha(reporteSeleccionado.brigadaAsignada.fechaAsignacion)}
                          </small>
                        </div>
                      )}
                ...
                {(reporteSeleccionado.estado === 'resuelto' || reporteSeleccionado.estado === 'completado') && reporteSeleccionado.fechaResolucion && (
                  <div className="reporte-campo">
                    ...
                    <div className="reporte-campo-valor">
                      <i className="fas fa-calendar-check"></i> {ReportesBasuraController.formatearFecha(reporteSeleccionado.fechaResolucion)}
                    </div>
                  </div>
                )}
