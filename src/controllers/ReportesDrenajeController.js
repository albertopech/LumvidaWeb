// src/controllers/ReportesDrenajeController.js - BACKEND LOGIC
...
/**
 * Controlador que contiene toda la lógica de negocio para reportes de drenaje obstruido
 */
export class ReportesDrenajeController {
  
  /**
   * Cargar todos los reportes de drenaje desde Firestore
  ...
  static async cargarReportes() {
    try {
      ...
      try {
        const q = query(
          reportesRef, 
          where("categoria", "==", CATEGORIAS.DRENAJE),
      ...
      console.log("Reportes de drenaje obstruido cargados:", reportesData.length);
  ...
  /**
   * Cargar brigadas disponibles para drenaje
   */
  ...
  static async generarPDF(reporte) {
    ...
    try {
      ...
      // Título
      pdf.text(`Reporte de Drenaje Obstruido - Folio: ${reporte.folio || 'N/A'}`, 15, 20);
      ...
      // Guardar el PDF
      pdf.save(`Reporte_Drenaje_${reporte.folio || reporte.id}.pdf`);
  ...
  /**
   * Análisis específico para reportes de drenaje obstruido
   */
  static analizarObstrucciones(reportes) {
    const zonas = {};
    const temporadas = {
      'Seca (Nov-Abr)': 0,
      'Lluvias (May-Oct)': 0
    ...
    reportes.forEach(reporte => {
      ...
      if (reporte.estado === 'pendiente') {
        zonas[zona].pendientes++;
      } else if (reporte.estado === 'resuelto' || reporte.estado === 'completado') {
        zonas[zona].resueltos++;
      }
      ...
      if (fecha >= treintaDiasAtras) {
        zonas[zona].reportesRecientes++;
      }
      
      // Análisis por temporada
      if (mes >= 5 && mes <= 10) {
        temporadas['Lluvias (May-Oct)']++;
      } else {
        temporadas['Seca (Nov-Abr)']++;
      }
    });
    
    // Calcular tiempo promedio de resolución por zona (simplificado)
    Object.keys(zonas).forEach(zona => {
      const zoneData = zonas[zona];
      if (zoneData.resueltos > 0) {
        // Estimación simple: asumimos 5-15 días promedio según la eficiencia
        const eficiencia = zoneData.resueltos / zoneData.total;
        zoneData.tiempoPromedioResolucion = Math.round(15 - (eficiencia * 10));
      }
      
      zoneData.porcentajeResolucion = zoneData.total > 0 ? 
        ((zoneData.resueltos / zoneData.total) * 100).toFixed(1) : 0;
    });
    
    return {
      zonasMasAfectadas: Object.values(zonas)
        .sort((a, b) => b.pendientes - a.pendientes)
        .slice(0, 5),
      analisisTemporadas: temporadas,
      zonasConMayorReincidencia: Object.values(zonas)
        .sort((a, b) => b.reportesRecientes - a.reportesRecientes)
        .slice(0, 3)
    };
  }
   ...
   */
  static exportarCSV(reportes, nombreArchivo = 'reportes_drenaje') {
    try {
      const headers = [
        'Folio',
        'Fecha',
