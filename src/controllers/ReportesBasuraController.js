// src/controllers/ReportesBasuraController.js - BACKEND LOGIC
...
/**
 * Controlador que contiene toda la lógica de negocio para reportes de basura acumulada
 */
export class ReportesBasuraController {
  
  /**
   * Cargar todos los reportes de basura desde Firestore
  ...
  static async cargarReportes() {
    try {
      ...
      try {
        const q = query(
          reportesRef, 
          where("categoria", "==", CATEGORIAS.BASURA),
  ...
  /**
   * Cargar brigadas disponibles para basura
   */
  ...
  static async generarPDF(reporte) {
    ...
    try {
      ...
      // Título
      pdf.text(`Reporte de Basura Acumulada - Folio: ${reporte.folio || 'N/A'}`, 15, 20);
      ...
      // Guardar el PDF
      pdf.save(`Reporte_Basura_${reporte.folio || reporte.id}.pdf`);
  ...
  /**
   * Análisis específico para reportes de basura acumulada
   */
  static analizarAcumulacion(reportes) {
    const zonas = {};
    ...
    reportes.forEach(reporte => {
      ...
      const fechaReporte = this.parsearFecha(reporte.fecha);
      if (!zonas[zona].ultimoReporte || fechaReporte > zonas[zona].ultimoReporte) {
        zonas[zona].ultimoReporte = fechaReporte;
      }
    });
    
    // Ordenar zonas por número de reportes pendientes (mayor problema primero)
    return Object.values(zonas)
      .sort((a, b) => b.pendientes - a.pendientes)
      .map(zona => ({
        ...zona,
        porcentajeResolucion: zona.total > 0 ? ((zona.resueltos / zona.total) * 100).toFixed(1) : 0,
        diasSinReporte: zona.ultimoReporte ? Math.floor((new Date() - zona.ultimoReporte) / (1000 * 60 * 60 * 24)) : null
      }));
  }
   ...
   */
  static exportarCSV(reportes, nombreArchivo = 'reportes_basura') {
    try {
      const headers = [
        'Folio',
        'Fecha',
