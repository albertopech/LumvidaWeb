// src/controllers/ReportesAlumbradoController.js - BACKEND LOGIC
...
/**
 * Controlador que contiene toda la lógica de negocio para reportes de alumbrado
 */
export class ReportesAlumbradoController {
  
  /**
   * Cargar todos los reportes de alumbrado desde Firestore
  ...
  static async cargarReportes() {
    try {
      ...
      try {
        const q = query(
          reportesRef, 
          where("categoria", "==", CATEGORIAS.ALUMBRADO),
  ...
  /**
   * Cargar brigadas disponibles para alumbrado
   */
  static async cargarBrigadasDisponibles() {
    try {
      const resultado = await BrigadaController.obtenerBrigadasDisponibles(CATEGORIAS.ALUMBRADO);
  ...
  static async generarPDF(reporte) {
    ...
    try {
      ...
      // Título
      pdf.text(`Reporte de Alumbrado Público - Folio: ${reporte.folio || 'N/A'}`, 15, 20);
      ...
      // Guardar el PDF
      pdf.save(`Reporte_Alumbrado_${reporte.folio || reporte.id}.pdf`);
   ...
   */
  static exportarCSV(reportes, nombreArchivo = 'reportes_alumbrado') {
    try {
      const headers = [
        'Folio',
        'Fecha',
